...
function parse(text) {
  ...
  const meta = Object.fromEntries(
    [...front.matchAll(/^(.+?):(.*)/gm)].map(([, k, v]) => [k.trim(), v.trim()]))
  return { ...meta, post: meta.post !== 'false', body: rest.join('---\n') }
}

const cache = {}
const load = id => cache[id] ??= fetch(`content/${id}.md`)
...
const tags = {
  image: (v, meta) => `<div class="gallery" tabindex="0" role="region" aria-label="Image gallery">${
    v.split(',').map(s => s.trim()).filter(Boolean)
      .map(s => `<img src="${s.startsWith('http') ? s : `images/${s}`}" alt="${esc(meta.title || '')}">`).join('')}</div>`
}
...
const focus = el => { el.setAttribute('tabindex', '-1'); el.focus({ preventScroll: true }) }
let posts = []

function show(e) {
  if (!e) return
  const i = posts.indexOf(e)
  const media = Object.keys(tags).filter(k => e[k]).map(k => tags[k](e[k], e)).join('')
  const header = e.date
...
if (entries) {
  ...
  document.getElementById('nav').insertAdjacentHTML('beforeend', ' ' + entries
    .filter(e => !e.post).map(p => `<a href="#/${p.id}">${esc(p.title)}</a>`).join(' '))
  render()
